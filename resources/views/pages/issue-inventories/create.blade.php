            ...
            <div class="col-lg-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <form method="post" class="form-horizontal" action="{{ route('issue-inventories.store') }}" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group">
                                <label class="col-sm-1 control-label">Issue No.</label>

                                <div class="col-sm-2">
                                    <input type="number" class="form-control @error('issue_no') is-invalid @enderror " name="issue_no" value="{{ old('issue_no') ?? $currentCode }}" required>
                                    @error('issue_no')
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-6">
                                    <div class="input-group">
                                        <select data-placeholder="Select Location" class="chosen-select" tabindex="2" id="locationSelect" name="location_code">
                                            <option value="">Select Location</option>
                                            @foreach ($locations as $location)
                                            <option value="{{ $location->code1 }}">
                                                {{ $location->code1 . ' - ' . $location->name1 }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
...
<script>
    $(document).ready(function() {
        var lastUpdatedField = ''; // Variable to track the last updated field

        function calculateValues() {
            var qty = parseFloat($('#qty').val());
            var value = parseFloat($('#value').val());
            var wgt_avg_rate = parseFloat($('#wgt_avg_rate').val());

            // Determine the last updated field
            if (isNaN(value)) {
                lastUpdatedField = 'wgt_avg_rate';
            } else if (isNaN(wgt_avg_rate)) {
                lastUpdatedField = 'value';
            }

            if (qty > 0 && value > 0 && lastUpdatedField === 'value') {
                // Calculate wgt_avg_rate if qty and value are provided and value was the last updated field
                var calculated_wgt_avg_rate = value / qty;
                $('#wgt_avg_rate').val(calculated_wgt_avg_rate);
            } else if (qty > 0 && wgt_avg_rate > 0 && lastUpdatedField === 'wgt_avg_rate') {
                // Calculate value if qty and wgt_avg_rate are provided and wgt_avg_rate was the last updated field
                var calculated_value = qty * wgt_avg_rate;
                $('#value').val(calculated_value);
            }
        }

        // Bind the calculateValues function to input events
        $('#qty, #value').on('input', calculateValues);
        $('#qty, #wgt_avg_rate').on('input', calculateValues);
    });
</script>
